...
spec = fft(filtered, length(filtered));
% 1/2 of the spectrum
spec = spec(1:length(filtered)/2+1);
Y = abs(spec); % 1/2 of the spectrum
...
ylabel('|P1(f)|');
    ...
    [~,locs] = findpeaks(Y,'MinPeakHeight',0.5,'MinPeakDistance',10);
    tf2 = mean(60./(locs/framerate)); % 60s/min / (pp/framerate) = bpm
    
% detect frequency
    [pks,locs] = findpeaks(Y,'MinPeakHeight',0.5,'MinPeakDistance',10);
    freq = f(locs);
    tf3 = mean(freq); % 60s/min / (pp/framerate) = bpm
